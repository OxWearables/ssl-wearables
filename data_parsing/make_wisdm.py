...
DATAFILES = '/Users/catong/repos/video-imu/data/wisdm/wisdm-dataset/raw/watch/accel/*.txt'
...
X, Y, T, P, = [], [], [], []
tmp = lambda x: float(x.strip(';'))
column_names = ['pid', 'code', 'time', 'x', 'y', 'z']

for datafile in tqdm(glob.glob(DATAFILES)):
    columns = ['pid', 'class_code', 'time', 'x', 'y', 'z']
    one_person_data = pd.read_csv(datafile, sep=",", header=None, converters={5: tmp}, names=column_names, parse_dates=['time'], index_col='time')
