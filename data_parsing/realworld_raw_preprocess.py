...
dataset_path = '/Users/catong/repos/video-imu/data/realworld/realworld2016_dataset/'
save_path = '/Users/catong/repos/video-imu/data/realworld/imu/'
sub_ids_list = list(range(1,16))
labels_list = ['climbingdown', 'climbingup', 'jumping', 'lying', 'running', 'sitting', 'standing', 'walking']
body_parts_list = ['chest', 'forearm', 'head', 'shin', 'thigh', 'upperarm', 'waist']
        ...
        for label in labels_list:
            ...
            if z_file.namelist()[0].endswith('zip'):         
                ...
                for i in range(1, len(z_file.namelist())+ 1):
                    sub_z_file = ZipFile(save_zip_path + 'acc_{}_{}_csv.zip'.format(ziplabel, i))
                    xyz_acc = cal_xyz_acc(sub_z_file, i, sub_id, ziplabel)
                    # save
                    file_name = '{}.{}.{}.npy'.format(sub_id, i-1, label)
                    np.save(save_path + file_name, xyz_acc)
                    sub_z_file.close()
                print('multiple: {}/{}; {} sessions in total'.format(sub_id, label, i))
            else:
