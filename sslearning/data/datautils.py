...
class Transform(object):
    def __init__(self, transformations=None, channel_wise=False, limits=[-1, 1]):
    ...
    def __call__(self, X):
        for aug_i in range(len(self.transformations)):
            if self.transformations[aug_i] == 'orientation_independent_transformation':
            ...
            if self.transformations[aug_i] == 'rescale':
                X=rescale_data(X, self.channel_wise, self.limits[0], self.limits[1])
        return X
...
def rescale_data(x, channel_wise=False, a=-1, b=1):
    ...
    if channel_wise is True:
        ...
        for i in range(x.shape[1]):
            x_[:, i]=np.multiply((b-a), (x_[:,i]-np.nanmin(x_[:, i]))/(np.nanmax(x_[:, i])-np.nanmin(x_[:, i])))+a
        x=x_
    else:
        sz=x.shape
        x=np.reshape(x, (-1,1))
        x=np.multiply((b-a), (x-np.nanmin(x))/(np.nanmax(x)-np.nanmin(x)))+a   
        x=np.reshape(x, sz)
    return x
