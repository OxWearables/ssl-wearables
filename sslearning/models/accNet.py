...
# adopted from https://github.com/pytorch/vision/blob/master/torchvision/models/vgg.py
...
class SSLNET(nn.Module):
    def __init__(self, output_size=2, input_size=1024,
                 number_nn=1024, flatten_size=1024):
        super(SSLNET, self).__init__()
        self.feature_extractor = cnn1()
        self.rotation_classifier = Classifier(input_size=flatten_size, output_size=output_size)
        #self.axis_classifier = Classifier(input_size=flatten_size, output_size=output_sizes[SWITCH_IDX])
...
def make_layers(cfg: List[Union[str, int]], batch_norm: bool = False) -> nn.Sequential:
    ...
    while i < len(cfg):
        ...
        if v == 'M':
            ...
            conv1d = nn.Conv1d(in_channels, v, kernel_size=my_kernel_size, padding=1)
            if batch_norm:
...
class CNN(nn.Module):
    ...
    def __init__(self, in_channels=3, num_filters_init=3):
        ...
        self.layer1 = ConvBNReLU(in_channels, num_filters_init,
                                 3, 1, 1, bias=False)  # 900 -> 225
        self.layer2 = ConvBNReLU(num_filters_init, num_filters_init,
                                 3, 1, 1, bias=False)  # 225 -> 56

        self.layer3 = ConvBNReLU(num_filters_init, num_filters_init * 2,
                                 3, 1, 1, bias=False)  # 56 -> 14
        self.layer4 = ConvBNReLU(num_filters_init * 2, num_filters_init * 2,
                                 3, 1, 1, bias=False)  # 14 -> 7

        self.layer5 = ConvBNReLU(num_filters_init * 2, num_filters_init * 4,
                                 12, 1, 1, bias=False)  # 7 -> 3
        self.layer6 = ConvBNReLU(num_filters_init * 4, num_filters_init * 4,
                                 12, 1, 1, bias=False)  # 6 -> 1

        self.layer7 = ConvBNReLU(num_filters_init * 4, num_filters_init * 8,
                                 36, 1, 1, bias=False)  # 6 -> 1
        self.layer8 = ConvBNReLU(num_filters_init * 8, num_filters_init * 8,
                                 36, 1, 1, bias=False)  # 7 -> 3
...
class CNNLSTM(nn.Module):
    def __init__(self, num_classes=2, in_cnn_channels=3,
                 num_cnn_filters_init=32, lstm_layer=3, lstm_nn_size=1024,
                 model_device='cpu', dropout_p=0, bidrectional=False, batch_size=10):
        super(CNNLSTM, self).__init__()
        self.feature_extractor = CNN(in_channels=in_cnn_channels, num_filters_init=num_cnn_filters_init)
        self.lstm = nn.LSTM(input_size=4608, hidden_size=lstm_nn_size, num_layers=lstm_layer, bidirectional=bidrectional)
...
cfgs: Dict[str, List[Union[str, int]]] = {
    'A': [64, 3,  64,  3,  'M',
          ...
          512, 3, 512, 3, 512, 3, 512, 3, 'M',
          1024, 30], # converted one FC to ConV
    'B': [64, 12, 64, 12, 'M',
          128, 24, 128, 24, 'M',
          256, 24, 256, 24, 'M',
          512, 24, 512, 24, 'M',
...
def _cnn(cfg: str, batch_norm: bool, pretrained: bool, **kwargs: Any) -> AccNet:
    if pretrained:
...
def cnn1(pretrained: bool = False, **kwargs: Any) -> AccNet:
    ...
    return _cnn('A', True, pretrained, classifier_input_size=classifier_input_size, **kwargs)
...
def cnn3(pretrained: bool = False, **kwargs: Any) -> AccNet:
    ...
    return _cnn('B', False, pretrained, classifier_input_size=classifier_input_size, **kwargs)
...
def cnn5(pretrained: bool = False, **kwargs: Any) -> AccNet:
    ...
    return _cnn('C', False, pretrained, classifier_input_size=classifier_input_size, **kwargs)
...
def cnnSmall(pretrained: bool = False, **kwargs: Any) -> AccNet:
    ...
    return _cnn('D', False, pretrained, classifier_input_size=classifier_input_size,
                classifier_layer_size=classifier_layer_size, **kwargs)
...
class ResBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels,
        ...
        self.conv1 = nn.Conv1d(in_channels, out_channels, kernel_size, stride, padding, bias=False, padding_mode='circular')
        self.conv2 = nn.Conv1d(out_channels, out_channels, kernel_size, stride, padding, bias=False, padding_mode='circular')
...
class Resnet(nn.Module):
    ...
    """
    def __init__(self, output_size=1, n_channels=3, is_eva=False, resnet_version=1, epoch_len=10, is_mtl=False):
        ...
        if resnet_version == 1:
            if epoch_len == 10:
                cgf = [(64, 5, 2, 5, 2, 2),
                      ...
                      (256, 5, 2, 5, 5, 1),
                      (512, 5, 2, 5, 5, 1),
                      (1024, 5, 0, 5, 3, 1)] # ResNet16
            else:
                cgf = [(64, 5, 2, 5, 3, 1),
                       (128, 5, 2, 5, 3, 1),
                       (256, 5, 2, 5, 5, 1),
                       (512, 5, 2, 5, 5, 1),
                       (1024, 5, 0, 5, 4, 0)] # 30 sec
        else:
            cgf = [(64, 5, 2, 5, 3, 1),
                   (64, 5, 2, 5, 3, 1),
                   (128, 5, 2, 5, 5, 1),
                   (128, 5, 2, 5, 5, 1),
                   (256, 5, 2, 5, 4, 0)] # smaller resnet
        in_channels = n_channels
        feature_extractor = nn.Sequential()
        for i, layer_params in enumerate(cgf):
            out_channels, conv_kernel_size, n_resblocks, resblock_kernel_size, downfactor, downorder = layer_params
            feature_extractor.add_module(
                f'layer{i+1}',
                Resnet.make_layer(in_channels, out_channels,
                                  conv_kernel_size, n_resblocks, resblock_kernel_size, downfactor, downorder)
            )
    ...
    def make_layer(in_channels, out_channels,
                   conv_kernel_size, n_resblocks, resblock_kernel_size,
                   downfactor, downorder=1):
        ...
        modules = [nn.Conv1d(in_channels, out_channels,
                             conv_kernel_size, 1, conv_padding, bias=False, padding_mode='circular')]
